...
export const useTokenRotation = () => {
    ...
    const updateAccessToken = async (refreshToken = tokenState?.refresh_token) => {
        ...
        if (access) {
            ...
            sessionStorage.setItem(SESSION_ACCESS_KEY, access.access);
    ...
    const login = useCallback(async (data: LoginRequest): Promise<LoginResponse | undefined> => {
        const response = await requestLogin(data.email, data.password);

        if (response) {
            setTokenState({ access_token: response.access, refresh_token: response.refresh });
            sessionStorage.setItem(STORAGE_AUTH_KEY, response.refresh);
            updateAccessToken(response.refresh);
        } else {
    ...
    return { login, logout, signup, accessToken: tokenState?.access_token };
};